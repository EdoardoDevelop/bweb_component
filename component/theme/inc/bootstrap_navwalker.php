...
class bs4Navwalker extends Walker_Nav_Menu
{
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $text_item = apply_filters( 'the_title', $item->title, $item->ID );
        if (strpos(strtolower($item->url), '#tax-') !== false){$item_has_children = true;}
        /**
        ...
        */
        ...
        $item_output .= '<span class="menu-text">' . $args->link_before . $text_item . $args->link_after . '</span>';
        $item_output .= '<span class="subtitle-text">' . $title_item_output . '</span>';

        
        $item_output .= '</a>';
        
        if (strpos(strtolower($atts['href']), '#tax-') !== false){
            $tx = str_replace("#tax-","",$atts['href']);
            ...
            foreach ( $categories as $category ) {
                $item_output .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" class=" dropdown-item"><span class="menu-text">' . esc_html( $category->name ) . '</span></a>';
            } 
